...
This document defines the required translation between internal engine language and user-facing company language.
...
| --- | --- | --- |
| `Graph` | Company operating model | Use when describing how the company is structured to do work. |
...
| `NodeRun` | Task execution | Use for a specific execution step inside an operation. |
| Prompt node | AI worker | Use when the user should think about an AI unit performing work. |
...
- Use `operating model` instead of `workflow definition` on top-level user surfaces.
...
- `Edit workflow` becomes `Update operating model`
