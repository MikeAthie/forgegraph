...
| P1.4 HITL first-class | Covered | `DecisionRecord` model, decision API, resume attempt identity in `backend/adapters/api/runs/views.py`, engine resume intent checks in `engine/application/usecase/runtime_intents_test.go`, backend resume/idempotency tests in `backend/tests/integration/adapters/test_run_api.py` | Expiry/escalation policy can be expanded, but durable decision state and duplicate-safe resume are implemented. |
