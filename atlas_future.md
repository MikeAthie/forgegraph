...
- Atlas performance evidence now supports local sandbox metric receipts for email, WhatsApp, social analytics, and landing-page analytics without live provider passwords. The pack declares read-only sandbox metric tools, and the generic performance service persists actual metrics, baselines, targets, variance, attribution scope, and optimization actions into backend-owned `MetricSnapshot`, `ReportRun`, and `EvaluationRun` evidence.
...
Managed OpenAI runtime is working again. The latest OpenAI-backed Atlas report-only gate passed cleanly with `gpt-5.4-mini`, while keeping `ATLAS_JUDGES_REQUIRE_SELLABLE=false`.

- Command used for that run: `PLAYWRIGHT_DOCKER_FRONTEND_URL=http://127.0.0.1:3001 LIVE_LLM_PROVIDER=openai LIVE_LLM_MODEL=gpt-5.4-mini OPENAI_BASE_URL=https://api.openai.com/v1 LIVE_LLM_JUDGE=true ATLAS_JUDGES_REQUIRE_SELLABLE=false npm run test:e2e:atlas:docker:local-llm`
- Result: Playwright passed in `6.6m` and persisted 13 backend-owned `atlas_rubric_scorecard_v1` evaluations.
- Overall paid-readiness: `4.6/5`, decision `sellable_with_minor_revisions`, hard fail `false`.
- Departments: strategy research `4.4`, brand/content `4.2`, channel execution `4.4`, CRM/lifecycle `4.0`, analytics/performance `4.0`, QA/compliance `4.4`, client/approval ops `4.4`.
- Processes: memory usefulness `4.4`, whiteboard usefulness `4.4`, snapshot recovery `5.0`, connector/tool honesty `4.8`, operation/reliability evidence `4.8`.
- Performance evidence: latest `MetricSnapshot 64f2153b-90e8-4537-af7c-717e3a04dc99` collected email, WhatsApp, social, and landing-page local sandbox metrics with zero blocked metric sources, baseline/target variance, attribution scope, and four optimization actions.
...
- The `3001` frontend override was a temporary workaround for a local Hermes WhatsApp bridge occupying `3000`. That bridge has since been moved to `3008`; future runs should use the normal Compose frontend on `127.0.0.1:3000` unless a preflight finds another conflict.
- Strict gate was not rerun yet to avoid another full paid-LLM pass; the latest report-only evidence now satisfies the numeric strict thresholds and is a credible candidate for a manual strict-gate run.
- Next improvement target: raise CRM/lifecycle and analytics/performance from `4.0/5` toward the large-LLM `4.5/5` quality target by improving lifecycle segmentation, retention/re-engagement planning, experiment thresholds, and final client-facing measurement narrative.
...
- Managed-OpenAI analytics/performance and overall paid-readiness now clear the strict numeric thresholds in report-only mode; run the strict gate only as a manual/release-candidate pass because it repeats the full paid-LLM flow.
...
curl -f http://127.0.0.1:8000/api/health || curl -f http://127.0.0.1:8000/health
...
Current local port expectations:

- ForgeGraph frontend: `127.0.0.1:3000`.
- ForgeGraph backend: `127.0.0.1:8000`.
- Hermes WhatsApp bridge, if running locally: `127.0.0.1:3008`, not `3000`.
- Any external process on `3000` should be treated as a preflight blocker, not worked around silently with a long Atlas run.
...
| INC-2026-06-03-01 | Atlas/analytics       | Analytics/performance was the weakest managed-OpenAI department score at `3.4/5`.                                                                                                | The prior run collected email metrics only; WhatsApp, social, and landing-page metric sources were blocked or undeclared, and the judge evidence omitted baseline/target variance and optimization actions.                                        | Added explicit local sandbox metric evidence for email, WhatsApp, social analytics, and landing-page analytics while preserving connector honesty. No live Gmail, WhatsApp, social, CMS, or analytics credentials are required, and production sends still require real connector approvals. | Rerun report-only with OpenAI and target analytics/performance `>= 3.5/5`; if still low, improve the actual measurement plan and final deliverable, not the judge threshold.                          |
| INC-2026-06-03-02 | System/judge harness  | The first OpenAI rerun was structurally successful but Playwright marked it flaky because earlier attempts rejected valid judge-plan aliases and safe runtime-invariant wording. | The judge used exact generic primitive aliases such as `kind`, `artifact`, `item`, `content`, and nested typed `items`; the invariant guard also flagged a safe recommendation to keep Redis/Kafka/WebSocket/engine state cache-or-transport-only. | Canonicalized exact primitive aliases to the five allowed ForgeGraph primitives, flattened nested typed plan items, accepted text aliases, and refined the invariant-hostility guard so protective cache/transport wording is not treated as a violation.                                    | Keep schema validation strict, but normalize exact equivalent field packaging when raw backend-owned judge runs prove the semantics are identical.                                                    |
| INC-2026-06-03-03 | System/local runtime  | Compose frontend could not bind `127.0.0.1:3000`, so the successful OpenAI Atlas rerun temporarily used a separate frontend container on `3001`.                                 | The local Hermes WhatsApp bridge owned port `3000` because its WhatsApp bridge default was still `3000`.                                                                                                                                           | Updated the local Hermes WhatsApp bridge defaults to `3008`, restarted the Hermes gateway, verified the bridge on `3008`, started the standard ForgeGraph frontend on `3000`, and removed the temporary `forgegraph-frontend-atlas` container.                                               | Add a local-E2E port preflight that verifies ForgeGraph owns `3000` and external local bridges use non-dev-server ports. Remember that a future Hermes update may overwrite the local bridge default. |
...
- [ ] Add a local-E2E port preflight that verifies `127.0.0.1:3000` belongs to the Compose frontend and known local bridges, including Hermes WhatsApp, are not occupying it.
...
- [x] Rerun managed OpenAI after the local sandbox metric evidence change and record whether analytics/performance clears `3.5/5`.
...
- [x] Add local sandbox multi-source metric evidence for analytics/performance without faking live provider credentials or production connector success.
...
- Performance metric snapshot ID, report run ID, evaluation run ID, source metrics, baseline/target variance, optimization actions, and optimization routing records.
