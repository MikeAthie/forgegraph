...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign in' link to open the login page so we can authenticate as test@example.com.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/header/div/div[2]/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the email and password fields with the provided test credentials and submit the login form, then wait for the app to finish redirecting.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Wait for the app to finish signing in, then navigate to /memory to open the first memory observation and inspect its detail panel to ensure tags/metadata are not editable.
