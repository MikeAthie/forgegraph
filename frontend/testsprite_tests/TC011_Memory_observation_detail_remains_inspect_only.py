...
async def run_test():
    ...
    try:
        ...
        # -> Open the login form by clicking the 'Sign in' link so we can authenticate with test@example.com.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/header/div/div[2]/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the email and password fields and submit the Sign in form, then navigate to /memory to locate the first memory observation.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Wait briefly for sign-in to complete, then navigate to /memory to load the memory list.
