...
def test_post_api_auth_register_user_registration():
    ...
    assert not any(k in json_resp for k in forbidden_token_keys), (
    ...
    assert duplicate_response.status_code == 400, (
        f"Expected 400 on duplicate but got {duplicate_response.status_code}"
    )
    ...
    for invalid_data in invalid_payloads:
        ...
        assert resp.status_code == 400, (
            f"Expected 400 for invalid data {invalid_data}, got {resp.status_code}"
        )
