...
def test_post_api_auth_login_user_authentication():
    ...
    invalid_login_payload = {"email": unique_email, "password": "WrongPassword!"}
    resp_invalid = requests.post(
        login_url, json=invalid_login_payload, headers=headers, timeout=TIMEOUT
    )
    assert resp_invalid.status_code == 401, (
        f"Expected 401 Unauthorized for invalid credentials but got {resp_invalid.status_code}"
    )
