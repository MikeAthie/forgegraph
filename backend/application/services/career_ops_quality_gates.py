...
def check_career_ops_packet_readiness(*, company: Graph, packet_version: AssetVersion) -> CareerOpsReadinessResult:
    ...
    checks["packet_belongs_to_company"] = "pass" if packet_version.asset.company_id == company.id else "blocked"
    source_refs = _source_refs(packet_version)
    checks["source_refs_present"] = "pass" if source_refs else "blocked"
    checks["no_internal_leakage"] = "pass" if not _has_internal_leakage(packet_version.provenance_json) else "blocked"
    checks["employer_identity_matches"] = "pass" if _has_company_opportunity_ref(company=company, packet_version=packet_version) else "blocked"
    ...
    checks["ats_simulation_report_present"] = "pass" if _is_ats_simulation_report(ats_simulation) else "blocked"
    checks["ats_human_review_minimum"] = "pass" if _ats_score_at_least(ats_simulation, 85) else "blocked"
    ...
    parseability_report = _ats_resume_parseability_report(company=company, packet_version=packet_version)
    checks["ats_resume_pdf_present"] = "pass" if ats_pdf_version is not None else "blocked"
    checks["ats_resume_pdf_mime_type"] = (
        "pass" if ats_pdf_version is not None and ats_pdf_version.mime_type == "application/pdf" else "blocked"
    )
    ...
    checks["ats_resume_pdf_exact_version_bound"] = (
        "pass" if _ats_pdf_bound_to_packet(company=company, packet_version=packet_version, pdf_version=ats_pdf_version) else "blocked"
    )
    checks["ats_resume_structure"] = "pass" if _has_required_ats_sections(tailored_resume) else "blocked"
    checks["claim_source_map"] = (
        "pass" if _has_claim_source_map(packet=payload, resume=tailored_resume, cover_letter=cover_letter) else "blocked"
    )
    checks["no_document_internal_leakage"] = (
        "pass" if not _has_document_internal_leakage(resume=tailored_resume, cover_letter=cover_letter) else "blocked"
    )
    checks["exact_version_approval_present"] = (
        "pass" if _has_exact_version_approval(company=company, packet_version=packet_version) else "blocked"
    )
    checks["side_effect_guard_disabled"] = (
        "pass" if _side_effect_guard_disabled(company=company, packet_version=packet_version) else "blocked"
    )
...
def _ats_resume_pdf_version(*, company: Graph, packet_version: AssetVersion) -> AssetVersion | None:
    ...
    if deliverable is None or deliverable.artifact is None:
        return None
    metadata = deliverable.metadata_json.get("career_ops", {}) if isinstance(deliverable.metadata_json, dict) else {}
...
def _ats_resume_parseability_report(*, company: Graph, packet_version: AssetVersion) -> dict[str, Any] | None:
    deliverable = _opportunity_deliverable(
        company=company,
        packet_version=packet_version,
    ...
    if deliverable is None or deliverable.artifact is None:
        return None
    version = AssetVersion.objects.filter(asset=deliverable.artifact).order_by("-version_number").first()
    if version is None:
        return None
...
def _has_required_ats_sections(resume: object) -> bool:
    ...
    return headings == list(ATS_REQUIRED_SECTIONS)
...
def _has_claim_source_map(*, packet: dict[str, Any], resume: object, cover_letter: object) -> bool:
    if not isinstance(resume, dict) or not isinstance(cover_letter, dict):
        return False
    if not _quality_flag(resume, "source_backed_claims") or not _quality_flag(cover_letter, "source_backed_claims"):
        return False
...
def _non_empty_claim_map(payload: dict[str, Any]) -> bool:
    ...
    if not isinstance(claim_map, list) or not claim_map:
        return False
    return all(isinstance(item, dict) and item.get("claim") and item.get("source_ref") for item in claim_map)
...
def _has_exact_version_approval(*, company: Graph, packet_version: AssetVersion) -> bool:
    opportunity_id = _payload_opportunity_id(packet_version)
    if not opportunity_id or not CompanyOpportunity.objects.filter(id=opportunity_id, company=company).exists():
        return False
    for decision in DecisionRecord.objects.filter(organization=company.organization, decision_type="human_approval"):
...
def _side_effect_guard_disabled(*, company: Graph, packet_version: AssetVersion) -> bool:
    if packet_version.asset.company_id != company.id:
        return False
    guard_sources: list[object] = [packet_version.provenance_json, packet_version.asset.metadata_json]
    guard_sources.extend(
        deliverable.metadata_json for deliverable in ServiceDeliverable.objects.filter(artifact=packet_version.asset)
    )
