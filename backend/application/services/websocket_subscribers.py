...
def update_run_websocket_subscriber_activity(
    *,
    ...
    if sent:
        subscriber["messages_sent"] = int(subscriber.get("messages_sent") or 0) + 1
    if dropped:
        subscriber["messages_dropped"] = int(subscriber.get("messages_dropped") or 0) + 1
    if filtered:
        subscriber["messages_filtered"] = int(subscriber.get("messages_filtered") or 0) + 1
    if heartbeat:
        subscriber["heartbeat_count"] = int(subscriber.get("heartbeat_count") or 0) + 1
    if slow_disconnect:
