...
def create_strategy_workstreams(*, user: User, whiteboard: WorkWhiteboard) -> list[TaskRoutingRecord]:
    definition = load_phase_definition(whiteboard=whiteboard, phase_id=DEFAULT_STRATEGY_PHASE_ID)
    return create_workstreams_from_definition(user=user, whiteboard=whiteboard, definition=definition)
...
def synthesize_strategy(*, user: User, whiteboard: WorkWhiteboard) -> tuple[Asset, AssetVersion]:
    return synthesize_phase_outputs(user=user, whiteboard=whiteboard, phase_id=DEFAULT_STRATEGY_PHASE_ID)
...
def strategy_state_payload(whiteboard: WorkWhiteboard) -> dict[str, Any]:
    ...
    route = advance_whiteboard_to_content_if_ready(user=whiteboard.created_by, whiteboard=whiteboard) if whiteboard.created_by_id else None
    return {
