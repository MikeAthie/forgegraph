...
ARCHETYPES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("AI Platform / LLMOps", ("observability", "eval", "pipeline", "monitoring", "reliability", "llmops")),
    ("Agentic / Automation", ("agent", "hitl", "orchestration", "workflow", "multi-agent", "automation")),
...
def evaluate_career_ops_posting(
    *,
    ...
    matched_requirements = _match_requirements(description=description, proof_points=proof_points, cv_summary=cv_summary)
    score = _score_posting(description=description, matched_requirements=matched_requirements, liveness_result=liveness.result)
    recommendation, tracker_status = _recommendation_for_score(score=score, liveness_result=liveness.result)
    ...
    evaluation = {
        ...
        "archetype": archetype,
        "blocks": {
            "A_role_summary": _role_summary(title=title, company=company, description=description, archetype=archetype),
            "B_cv_match": {
            ...
            "E_customization_plan": _customization_plan(archetype=archetype, matched_requirements=matched_requirements),
            "F_interview_plan": _interview_plan(archetype=archetype, matched_requirements=matched_requirements),
            "G_posting_legitimacy": _legitimacy_block(liveness=liveness, description=description),
        },
        "draft_application_answers": _draft_application_answers(company=company, title=title, matched_requirements=matched_requirements)
...
def _posting_text(posting: dict[str, Any]) -> str:
    return str(posting.get("description") or posting.get("body_text") or posting.get("jd_text") or "").strip()
...
def _detect_archetype(text: str) -> dict[str, Any]:
    ...
    return {"primary": primary if primary_hits else "General AI / Software", "secondary": secondary, "signals": hits}


def _match_requirements(*, description: str, proof_points: list[str], cv_summary: str) -> list[dict[str, str]]:
...
def _score_posting(*, description: str, matched_requirements: list[dict[str, str]], liveness_result: str) -> float:
...
def _role_summary(*, title: str, company: str, description: str, archetype: dict[str, Any]) -> dict[str, Any]:
    lowered = description.casefold()
    remote = "remote" if "remote" in lowered else "hybrid" if "hybrid" in lowered else "onsite_or_unspecified"
    seniority = "senior" if re.search(r"\b(senior|staff|lead|principal)\b", f"{title} {description}", re.I) else "unspecified"
    return {
        "company": company,
        "role_title": title,
...
def _gaps(*, description: str, matched_requirements: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for keyword in _keywords(description):
        if keyword.casefold() not in matched:
            gaps.append({"requirement": keyword, "mitigation": "Address only with real adjacent evidence or leave unclaimed."})
        if len(gaps) == 5:
            break
    return gaps
...
def _level_strategy(*, title: str, description: str) -> dict[str, str]:
    level = "senior" if re.search(r"\b(senior|staff|lead|principal)\b", f"{title} {description}", re.I) else "unspecified"
    return {
...
def _comp_research(description: str) -> dict[str, Any]:
    salary = re.search(r"(?:\$|€|£)\s?\d{2,3}[kK]?(?:\s?[-–]\s?(?:\$|€|£)?\s?\d{2,3}[kK]?)?", description)
    return {
...
def _customization_plan(*, archetype: dict[str, Any], matched_requirements: list[dict[str, str]]) -> list[dict[str, str]]:
    return [
        {
            "section": "Summary",
        ...
        },
        {
            ...
            "proposed_change": "Prioritize matched proof points: " + ", ".join(match["requirement"] for match in matched_requirements[:3]),
            "why": "Career-Ops emphasizes exact JD requirement to CV evidence mapping.",
        },
    ]


def _interview_plan(*, archetype: dict[str, Any], matched_requirements: list[dict[str, str]]) -> list[dict[str, str]]:
    if not matched_requirements:
        return [{"requirement": "general", "story_prompt": "Prepare a STAR+R story from verified cv_source evidence."}]
    return [
        {
...
def _legitimacy_block(*, liveness: Any, description: str) -> dict[str, Any]:
    ...
    if len(description) > 300:
        quality_signals.append({"signal": "description_quality", "finding": "specific content present", "weight": "Positive"})
    else:
        quality_signals.append({"signal": "description_quality", "finding": "short or missing JD content", "weight": "Concerning"})
    quality_signals.append({"signal": "liveness", "finding": liveness.reason, "weight": "Positive" if liveness.result == "active" else "Concerning"})
    tier = "High Confidence" if liveness.result == "active" else "Suspicious" if liveness.result == "expired" else "Proceed with Caution"
    return {"assessment": tier, "liveness": liveness.as_dict(), "signals": quality_signals, "ethical_note": "Signals prioritize candidate time; they are not accusations."}


def _draft_application_answers(*, company: str, title: str, matched_requirements: list[dict[str, str]]) -> list[dict[str, str]]:
    evidence = matched_requirements[0]["cv_evidence"] if matched_requirements else "verified CV evidence"
    return [
        {"question": GENERIC_QUESTIONS[0], "answer": f"This {title} role maps directly to work I can evidence: {evidence}."},
        {"question": GENERIC_QUESTIONS[1], "answer": f"I am evaluating {company} because the role shows concrete overlap with my verified experience."},
        {"question": GENERIC_QUESTIONS[2], "answer": f"A relevant proof point is: {evidence}."},
        {"question": GENERIC_QUESTIONS[3], "answer": "The fit comes from the intersection of the JD requirements and the cited CV evidence above."},
        {"question": GENERIC_QUESTIONS[4], "answer": "Found through a CareerOps-style opportunity review and selected after scoring against my criteria."},
    ]


def _source_refs(*, posting: dict[str, Any], matched_requirements: list[dict[str, str]]) -> list[dict[str, str]]:
    refs = []
    if posting.get("url"):
        refs.append({"type": "job_url", "url": str(posting["url"])})
    refs.extend({"type": "cv_source", "requirement": match["requirement"]} for match in matched_requirements)
    return refs
