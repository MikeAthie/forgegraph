...
ONBOARDING_DEPARTMENTS = {
    "account-intake": {
        ...
        "fields": {"objective", "offer", "timeline", "approval_owner", "inventory_price_margin_constraints"},
    },
    "strategy": {
        "name": "Strategy",
...
def create_or_resume_whiteboard(  # noqa: C901
    *,
    ...
    if source_message is not None and source_message.company_id != company.id:
        raise WorkWhiteboardError("company_mismatch", "Source message belongs to a different company.")
    if service_engagement is not None and service_engagement.company_id != company.id:
        raise WorkWhiteboardError("company_mismatch", "Service engagement belongs to a different company.")
    ...
    if key:
        existing = _whiteboard_queryset().filter(organization=company.organization, idempotency_key=key).first()
        if existing is not None:
            return existing
    ...
    whiteboard = WorkWhiteboard(
        ...
        client_name=str(data.get("client_name") or company.name or "")[:255],
        ...
        constraints_json=dict(data.get("constraints") or {}),
        target_audience_json=dict(data.get("target_audience") or {}),
        brand_context_json=dict(data.get("brand_context") or {}),
        product_context_json=dict(data.get("product_context") or {}),
        channel_context_json=dict(data.get("channel_context") or {}),
        known_facts_json=dict(data.get("known_facts") or {}),
        assumptions_json=list(data.get("assumptions") or []),
        metadata_json=dict(data.get("metadata") or {}),
        idempotency_key=key,
        created_by=created_by,
    )
    whiteboard.missing_fields_json = compute_whiteboard_missing_fields(whiteboard)
    whiteboard.completion_score = compute_whiteboard_completion_score(whiteboard.missing_fields_json)
...
def initialize_whiteboard_from_message(
    *,
    ...
    if message.company is None:
        raise WorkWhiteboardError("company_required", "Whiteboards require a company-scoped message.")
...
def route_account_intake_clarification(
    *,
    ...
    if message.company is None:
        raise WorkWhiteboardError("company_required", "Clarification routing requires a company-scoped message.")
    department = _ensure_onboarding_department(
        company=message.company,
        slug="account-intake",
        name="Account Intake",
...
def update_whiteboard_field(
    *,
    ...
    if not _can_mutate_whiteboard(user=user, whiteboard=whiteboard):
        raise WorkWhiteboardError("permission_denied", "You do not have permission to update this whiteboard.")
    ...
    json_map = {
        ...
        "channel_context": "channel_context_json",
        "known_facts": "known_facts_json",
        "metadata": "metadata_json",
    }
    for key, attr in json_map.items():
        if key in data:
            setattr(whiteboard, attr, dict(data.get(key) or {}))
            update_fields.append(attr)
    if "assumptions" in data:
        whiteboard.assumptions_json = list(data.get("assumptions") or [])
        update_fields.append("assumptions_json")
    whiteboard.missing_fields_json = compute_whiteboard_missing_fields(whiteboard)
    whiteboard.completion_score = compute_whiteboard_completion_score(whiteboard.missing_fields_json)
    update_fields.extend(["missing_fields_json", "completion_score"])
    whiteboard.full_clean()
...
def mark_whiteboard_ready_for_strategy(*, user: User, whiteboard: WorkWhiteboard) -> TaskRoutingRecord:
    if not _can_mutate_whiteboard(user=user, whiteboard=whiteboard):
        raise WorkWhiteboardError("permission_denied", "You do not have permission to update this whiteboard.")
    with transaction.atomic():
        ...
        whiteboard.completion_score = compute_whiteboard_completion_score(whiteboard.missing_fields_json)
        whiteboard.full_clean()
        whiteboard.save(update_fields=["status", "missing_fields_json", "completion_score", "updated_at"])
        department = _ensure_onboarding_department(
            company=whiteboard.company,
            slug="strategy",
            name="Strategy",
        ...
        record = route_event_to_department(
            ...
            department=department,
            user=user,
            event_type="whiteboard.ready_for_strategy",
            trigger_type="whiteboard.ready_for_strategy",
            communication_thread=whiteboard.communication_thread,
            communication_message=whiteboard.source_message,
            service_engagement=whiteboard.service_engagement,
            reason="Whiteboard is ready for strategy intake.",
            status="queued",
            priority="normal",
            idempotency_key=f"whiteboard:{whiteboard.id}:ready-for-strategy",
            metadata={"whiteboard_id": str(whiteboard.id), "completion_score": whiteboard.completion_score},
        )
    refresh_whiteboard_redis_snapshot(whiteboard)
    return record
...
def compute_whiteboard_missing_fields(whiteboard: WorkWhiteboard) -> list[str]:  # noqa: C901
    ...
    if not _has_any(known_facts, ["inventory_price_margin_constraints", "inventory", "price", "margin"]):
...
def extract_known_fields_from_message(message: CommunicationMessage) -> dict[str, Any]:
    ...
    channels = [
        channel
        for channel in ["whatsapp", "twilio", "brevo", "email", "instagram", "facebook", "social", "landing page"]
        if channel in lower
    ]
    ...
    return {
        "request_type": request_type,
        "client_name": message.company.name if message.company_id else "",
...
def whiteboard_payload(
    ...
    *,
    ...
    operator = _can_view_internal(user=user, whiteboard=whiteboard) if include_internal is None else include_internal
    payload = {
        "id": str(whiteboard.id),
        "organization_id": str(whiteboard.organization_id),
        "company_id": str(whiteboard.company_id),
        "service_engagement_id": str(whiteboard.service_engagement_id) if whiteboard.service_engagement_id else None,
        "communication_thread_id": str(whiteboard.communication_thread_id) if whiteboard.communication_thread_id else None,
        "source_message_id": str(whiteboard.source_message_id) if whiteboard.source_message_id else None,
...
def refresh_whiteboard_redis_snapshot(whiteboard: WorkWhiteboard) -> None:
    ...
    if not _use_cache_snapshot_store():
        try:
            ...
            pass
    cache.set(key, serialized, timeout=None)
...
def _active_whiteboard_for(
    *,
    ...
    queryset = _whiteboard_queryset().filter(company=company, status__in=ACTIVE_WHITEBOARD_STATUSES)
