    ...
    "skills": [
        {
            ...
            "items": ["Python", "FastAPI", "Django", "Go", "REST APIs", "service architecture"],
        },
        {
        ...
        },
        {
            ...
            "items": ["React", "Next.js", "TypeScript", "dashboards", "internal tools"],
        },
        {
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument(
            ...
            default=os.environ.get(
                "CAREER_OPS_WHATSAPP_CHAT_ID", os.environ.get("WHATSAPP_CHAT_ID", "")
            ),
    ...
    def handle(self, *args: object, **options: object) -> None:
        ...
        raw_max_jobs = options.get("max_jobs")
        max_jobs = max(
            1,
            min(
                int(raw_max_jobs)
                if isinstance(raw_max_jobs, str | bytes | bytearray | int | float)
                else 5,
                10,
            ),
        )
        ...
        for run_info in discovery.get("runs", []):
            ...
            if packet_version_id:
                ...
                asset_versions = _asset_versions_for_packet(
                    company=company, packet_version=packet_version
                )
            packets.append(
                {
        ...
        if bool(options.get("send_whatsapp")):
            whatsapp_delivery = _send_whatsapp_delivery(
                ...
                output_dir=str(
                    options.get("delivery_output_dir") or "/app/.hermes/career_ops_e2e_delivery"
                ),
                idempotency_key=idempotency_key,
            )
        ...
        payload = {
            ...
            "docker_command": _docker_command(
                max_jobs=max_jobs, company_name=company.name, user_email=actor.email
            ),
...
def _ensure_company(*, actor: User, name: str) -> Graph:
    ...
    company = Graph.objects.filter(
        owner=actor, organization=actor.default_organization, name=name
    ).first()
    if company is None:
...
def _asset_versions_for_packet(
    *, company: Graph, packet_version: AssetVersion
    ...
    result: dict[str, str | None] = dict.fromkeys(mapping.values())
...
def _counts(*, company: Graph) -> dict[str, int]:
    return {
        "runs": Run.objects.filter(
            organization=company.organization, input_json__career_ops__dry_run=True
        ).count(),
        "signals": CompanySignal.objects.filter(
            company=company, domain_context="career_ops"
        ).count(),
...
def _send_whatsapp_delivery(
    *,
    ...
    if organization is None:
        raise ValueError("CareerOps WhatsApp delivery requires an organization-scoped company.")
    chat_id = chat_id.strip()
    if not chat_id:
        raise ValueError(
            "--send-whatsapp requires --whatsapp-chat-id or CAREER_OPS_WHATSAPP_CHAT_ID."
        )
    ...
    exported = _export_cv_pdfs_for_delivery(
        packets=packets, output_dir=output_dir, bridge_url=bridge_url
    )
    ...
    link_lines = [
        f"{idx}. {item['company']} — {item['role_title']}: {item['job_url']}"
        for idx, item in enumerate(exported, start=1)
    ]
    summary_text = (
        "CareerOps E2E ran inside ForgeGraph. CV PDFs are attached above; application links:\n"
        + "\n".join(link_lines)
    )
    ...
    missing_paths = [
        ...
        if not Path(item["container_file_path"]).exists()
    ]
    if missing_paths:
    ...
    for idx, item in enumerate(exported, start=1):
        ...
        media_response = requests.post(
            f"{bridge_url}/send-media", json=media_payload, headers=headers, timeout=120
        )
        ...
        CommunicationAttachment.objects.get_or_create(
            message=message, artifact_revision=pdf_version
        )
...
def _export_cv_pdfs_for_delivery(
    *, packets: list[dict[str, Any]], output_dir: str, bridge_url: str
    ...
    for packet in packets:
        ...
        if not encoded:
            raise ValueError(
                f"ATS resume PDF AssetVersion {version.id} has no inline_content_base64."
            )
        pdf_bytes = base64.b64decode(str(encoded))
        career_ops = (
            version.asset.metadata_json.get("career_ops", {})
            if isinstance(version.asset.metadata_json, dict)
            else {}
        )
        ...
        opp_meta = (
            opportunity.metadata_json.get("career_ops", {})
            if opportunity and isinstance(opportunity.metadata_json, dict)
            else {}
        )
        company_name = str(
            opp_meta.get("employer_name")
            or (opportunity.company.name if opportunity else "Company")
        )
        role_title = str(
            opp_meta.get("role_title") or (opportunity.title if opportunity else "Role")
        )
