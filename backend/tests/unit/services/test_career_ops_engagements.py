...
def _create_company(user: User) -> Graph:
    ...
    return cast(Graph, Graph.objects.create(owner=user, organization=organization, name="CareerOps Engagement Co"))
...
def test_ensure_career_ops_application_engagement_is_idempotent(user: User) -> None:
    ...
    assert ServiceCatalogItem.objects.filter(organization=company.organization, slug="career-ops-application-packet").count() == 1
    assert ServiceEngagement.objects.filter(company=company, source_key=f"career-ops:{company.id}:application-pipeline").count() == 1
